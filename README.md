...
* `node['graphite']['graphite_role']` - chef role name for graphite instances, used by the *federated* recipe (defaults to "graphite")
* `node['graphite']['graphite_query']` - search query to find other nodes, used by the *federated* recipe (defaults to using the role above)
